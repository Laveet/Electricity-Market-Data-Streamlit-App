...
#         # Merge generation + load for renewable share computation
#         ren_share = 0.0
#         if not gen_df.empty:
#             ren_share = AdditionalAnalyticsFeatures.calculate_renewable_share(gen_df)
...
#                 with sc_col2:
#                     if "price_eur_mwh" in feature_merged.columns:
#                         # Combine wind onshore and offshore for total wind series if both exist
#                         wind_cols = [c for c in ["wind_onshore_mw", "wind_offshore_mw"] if c in feature_merged.columns]
#                         if wind_cols:
#                             feature_merged["total_wind_mw"] = feature_merged[wind_cols].sum(axis=1)
#                             fig_wind_sc = px.scatter(
...
# # # --- MAIN DASHBOARD TABS ---
# # tab1, tab2, tab3 = st.tabs(["📊 Price & Load Analytics", "🌱 Generation Mix & Residual Load", "🔄 Cross-Border Spreads"])
